...
%         dataPts(5:7,:) = dataPts(5:7,:)  ./ maxTrans ;
    ...
    case 3
        ...
        maxD =  max(max(dataPts(4,:)) -  min(dataPts(4,:)));
        dataPts(4,:) = dataPts(4,:)  ./ maxD;
        d_max = max(dataPts',[],1);
        d_min = min(dataPts',[],1);
        L = d_max - d_min;
        bandWidth = max(L)/grid;
    case 4
        ...
        maxD =  max(max(dataPts(7,:)) -  min(dataPts(7,:)));
        dataPts(1:3,:) = dataPts(1:3,:)  ./ pi;
        dataPts(7,:) = dataPts(7,:)  ./ maxD;
%         dataPts(1:3,:) = dataPts(1:3,:)  ./ pi ./ 2;
%         dataPts(7,:) = dataPts(7,:)  ./ boxSize;
        ...
        bandWidth = max(L)/grid;
    case 5
        save('D:\\baseballBat_RPS8.mat','dataPts')
        maxS = max(dataPts(1,:));
        maxD = max(dataPts(8,:));
        dataPts(1,:) = dataPts(1,:)  ./ maxS;
        dataPts(2:4,:) = dataPts(2:4,:)  ./ pi;
        dataPts(8,:) = dataPts(8,:)  ./ maxD;
%         dataPts(1:3,:) = dataPts(1:3,:)  ./ pi ./ 2;
%         dataPts(7,:) = dataPts(7,:)  ./ boxSize;
        d_max = max(dataPts',[],1);
        d_min = min(dataPts',[],1);
        L = d_max - d_min;
        bandWidth = max(L(2:8))/grid;
    case 6
...
%*** If they want the cluster2data cell find it for them
    ...
    switch mode
        case -1
            clustCent(1,:) = clustCent(1,:);
            clustCent(2,:) = clustCent(2,:);
        case 0 
...
%             clustCent(4:6,:) = clustCent(4:6,:);
        ...
        case 3
            clustCent(1:3,:) = clustCent(1:3,:);
             clustCent(4,:) = clustCent(4,:) .* maxD;
        case 4
            clustCent(1:3,:) = clustCent(1:3,:) .* pi ;
            %%Bug?
            clustCent(7,:) = clustCent(7,:) .* maxD;
        case 5
            clustCent(1,:) = clustCent(1,:) .* maxS;
            clustCent(2:4,:) = clustCent(2:4,:) .* pi ;
            %%Bug?
            clustCent(8,:) = clustCent(8,:) .* maxD;
        case 6
            clustCent(1:3,:) = clustCent(1:3,:) .* pi .* 2;
    end
...
switch mode
    ...
    case 1
        ...
        save('D:\\plane2Cent.mat','clustCent')
    case 2
    ...
    case 5
        save('D:\\baseballBat_clusterRSS.mat','data2cluster')
    case 6
